      ...
      <v-card color="white" class="pa-3 no_border" elevation="0">
        <v-row>
          <v-col sm="8" cols="12" class="py-0">
            <h2 class="mb-0 font_020">{{ $t("sale_quote_by_product_service") }}</h2>
        ...
        <v-row>
          ...
          <v-col sm="1" cols="12" class="py-0 mt-6">
            <v-btn class="btn_search" color="primary" @click="onSearch('Sale Quote')">
        ...
        <v-row>
          <v-col sm="12" cols="12" class="py-0 pt-0">
            ...
            <gridFullScreen>
              <template>
                <kendo-datasource
                  ref="gridReportQuoteByProductServiceDS"
                  :type="'JSON'"
                  :data="transactions"
                  :group="group"
                  :server-paging="false"
                />
                <kendo-grid
                  id="gridReportQuoteByProductService"
                  class="grid-function"
                  :data-source-ref="'gridReportQuoteByProductServiceDS'"
                  :editable="false"
                  :groupable="true"
                  :sortable="true"
                  :column-menu="true"
                  ...
                  :excel-file-name="'ReportSaleQuoteByProductService.xlsx'"
                ...
                >
                  ...
                  <kendo-grid-column
                    ...
                    :title="$t('so_price')"
                    :width="150"
                    :template="'<span>#=kendo.toString(soPrice, decimalFormat)#</span>'"
                    :filterable="false"
                    :attributes="{ style: 'text-align: right; ' }"
                    :headerAttributes="{
                      style: 'background-color: #EDF1F5; text-align:right',
        ...
        <div style="overflow: auto" class="byQuotePS-pager"></div>
      </v-card>
    </v-col>
    <v-dialog persistent v-model="dialogQuoteStatus" max-width="450px">
      <v-card>
        <div class="modal_header">
          <v-card-title>{{ $t("change_status") }}: {{ qRefNum }}</v-card-title>
...
export default {
  ...
  data: () => ({
    ...
    locations: [],
    location: {},
    objStatus: ["Open", "Contracted", "Abandon", "Decline"],
  ...
  methods: {
    ...
    async saveClose() {
      try {
        if (this.status___ !== "" && this.quoteId !== "") {
          new Promise((resolve) => {
            setTimeout(() => {
              resolve("resolved");
    ...
    dataBound: function (e) {
      const grid = kendo.jQuery("#gridReportQuoteByProductService").data("kendoGrid");
    ...
    pdfExport: function (e) {
      // const pivot = this.$refs.gridReportQuoteByProductServiceDS.kendoWidget()
      // pivot.saveAsPDF()
    ...
    goEdit(e) {
      ...
      this.qRefNum = dataItem.referenceNo || "";
      this.quoteId = dataItem.id || "";
    ...
    rowNumberTmpl(dataItem) {
      let ds = this.$refs.gridReportQuoteByProductServiceDS.kendoWidget(),
    ...
    async loadPage(that) {
      $(".byQuotePS-pager").html(`
                <label class="label byQuotePS-item-returned">0</label>
                <div style="display: flex; flex-direction: 'row'">
                    <button class="byQuotePS-next-page" title="previous" style="width: 24px; font-size: 19px;"> < </button>
                    <ul class="byQuotePS-page-number" style="display: flex;list-style: none; padding-left: 0px;">
                        <li style="padding: 6px;"  data-page="1" class="byQuotePS-page-link text-bold-link" >1</li>
                    </ul>
                    <button class="byQuotePS-next-page" title="next" style="width: 24px; font-size: 19px;"> > </button>
                </div>
            `);
      $(document).on("click", ".byQuotePS-page-link", function () {
        const page = $(this).data("page");
        $(".byQuotePS-page-link").removeClass("text-bold-link");
        $(this).addClass("text-bold-link");
        that.backToPage(that, page - 1);
        window.console.log(page - 1);
      });
      // $(document).on('click', '.byQuotePS-next-page', function () {
      $(".byQuotePS-next-page").click(function () {
        const page_number = $(this)
          .parent()
          .children(".byQuotePS-page-number")
          .find("li")
          .last()
          .data("page");
        window.console.log("byQuotePS-page_number", page_number);
        ...
        if (isPrevious == "previous") {
          ...
          $(".byQuotePS-page-link").removeClass("text-bold-link");
          $(`.byQuotePS-page-link:eq(${currectpage - 1})`).addClass("text-bold-link");
          ...
          $(this)
            ...
            .children(".byQuotePS-page-number")
            .append(
              `<li class="byQuotePS-page-link text-bold-link" style="padding: 6px;" data-page="${
                page_number + 1
              }">${page_number + 1}</li>`
            );
        }
    ...
    itemRetuned(that) {
      ...
      $(".byQuotePS-item-returned").text("Items returned (" + totalRow + ")");
    },
    disabledNextPage(value) {
      $(".byQuotePS-next-page").prop("disabled", value);
    },
    ...
    refreshGridData(data) {
      $("#gridReportQuoteByProductService").data("kendoGrid").dataSource.data([]);
      $("#gridReportQuoteByProductService").data("kendoGrid").dataSource.data(data);
    },
