    ...
    <v-col sm="12" cols="12" :class="isFund ? 'px-4 pt-0' : 'grayBg px-6'">
      <v-card color="white" class="pa-3 no_border" elevation="0">
        ...
        <v-row>
          ...
          <v-col sm="1" cols="12" class="py-0 mt-6">
            <v-btn class="btn_search" color="primary" @click="onSearch('Invoice funnel')">
        ...
        <v-row>
          <v-col sm="12" cols="12" class="py-0 pt-0">
            ...
            <gridFullScreen>
              <template>
                <kendo-datasource
                  ref="gridReportInvoiceByCustomerDS"
                  :type="'JSON'"
                  :data="invoiceTxns"
                  :group="group"
                  :server-paging="false"
                ...
                <kendo-grid
                  ...
                  class="grid-function"
                  :data-source-ref="'gridReportInvoiceByCustomerDS'"
                  :editable="false"
                  :groupable="true"
                  :sortable="true"
                  :column-menu="true"
                  ...
                  :excel-file-name="'ReportInvoiceByCustomer.xlsx'"
        ...
        <div style="overflow: auto" class="funnel-by-invoice-pager"></div>
      </v-card>
    </v-col>
  </v-row>
</template>
...
export default {
  ...
  data: () => ({
    ...
    locations: [],
    location: {},
    objStatus: ["Open", "Paid", "Partially Paid"],
  ...
  methods: {
    onKChange(e) {
      this.kChecked = e.target.value;
      window.console.log("onKChange", this.kChecked);
      // if (this.kChecked) {
    ...
    pdfExport: function (e) {
      // const pivot = this.$refs.gridReportInvoiceByCustomerDS.kendoWidget()
      // pivot.saveAsPDF()
    ...
    rowNumberTmpl(dataItem) {
      let ds = this.$refs.gridReportInvoiceByCustomerDS.kendoWidget(),
      ...
      new Promise((resolve) => {
        setTimeout(() => {
          ...
          if (this.status_ === "Open") {
            iStatus = 1;
          } else if (this.status_ === "Partially Paid") {
            iStatus = 2;
          } else if (this.status_ === "Paid") {
            iStatus = 3;
          }
          this.params = {
            search: "",
    ...
    // },
    async loadPage(that) {
      $(".funnel-by-invoice-pager").html(`
                <label class="label byFunnel-item-returned">0</label>
                <div style="display: flex; flex-direction: 'row'">
                    <button class="byFunnel-next-page" title="previous" style="width: 24px; font-size: 19px;"> < </button>
                    <ul class="byFunnel-page-number" style="display: flex;list-style: none; padding-left: 0px;">
                        <li style="padding: 6px;"  data-page="1" class="byFunnel-page-link text-bold-link" >1</li>
                    </ul>
                    <button class="byFunnel-next-page" title="next" style="width: 24px; font-size: 19px;"> > </button>
                </div>
            `);
      $(document).on("click", ".byFunnel-page-link", function () {
        const page = $(this).data("page");
        $(".byFunnel-page-link").removeClass("text-bold-link");
        $(this).addClass("text-bold-link");
        that.backToPage(that, page - 1);
        window.console.log(page - 1);
      });
      // $(document).on('click', '.byFunnel-next-page', function () {
      $(".byFunnel-next-page").click(function () {
        const page_number = $(this)
          .parent()
          .children(".byFunnel-page-number")
          .find("li")
          .last()
          .data("page");
        window.console.log("byFunnel-page_number", page_number);
        ...
        if (isPrevious == "previous") {
          ...
          $(".byFunnel-page-link").removeClass("text-bold-link");
          $(`.byFunnel-page-link:eq(${currectpage - 1})`).addClass("text-bold-link");
          ...
          $(this)
            ...
            .children(".byFunnel-page-number")
            .append(
              `<li class="byFunnel-page-link text-bold-link" style="padding: 6px;" data-page="${
                page_number + 1
              }">${page_number + 1}</li>`
            );
        }
    ...
    itemRetuned(that) {
      ...
      $(".byFunnel-item-returned").text("Items returned (" + totalRow + ")");
    },
    disabledNextPage(value) {
      $(".byFunnel-next-page").prop("disabled", value);
    },
