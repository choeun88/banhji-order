      ...
      <v-card color="white" class="pa-3 no_border" elevation="0">
        <v-row>
          <v-col sm="8" cols="12" class="py-0">
            <h2 class="mb-0 font_020">{{ $t("sale_order_customer") }}</h2>
        ...
        <v-row>
          ...
          <v-col sm="1" cols="12" class="py-0 mt-6">
            <v-btn class="btn_search" color="primary" @click="onSearch('Sale Order')">
        ...
        <v-row>
          <v-col sm="12" cols="12" class="py-0 pt-0">
            ...
            <gridFullScreen>
              <template>
                <kendo-datasource
                  ref="gridReportSaleOrderByCustomerDS"
                  :type="'JSON'"
                  :data="transactions"
                  :group="group"
                  :server-paging="false"
                />
                <kendo-grid
                  id="gridReportSaleOrderByCustomer"
                  class="grid-function"
                  :data-source-ref="'gridReportSaleOrderByCustomerDS'"
                  :editable="false"
                  :groupable="true"
                  :sortable="true"
                  :column-menu="true"
                  ...
                  :excel-file-name="'ReportSaleQuoteByCustomer.xlsx'"
                ...
                >
                  <kendo-grid-column
                    ...
                    :command="[
                      { text: $t('convert'), click: goToInvoice },
                      { text: $t('status'), click: goEdit },
                    ]"
                    :headerAttributes="{
                  ...
                  />
                  <kendo-grid-column
                    :field="$t('doReferenceNo')"
                    :title="$t('do_number')"
                    :width="180"
                    :hidden="false"
                    :encoding="false"
                    :template="'<span>#=doReferenceNo || ``#</span>'"
                    :sortable="true"
                    :headerAttributes="{ style: 'background-color: #EDF1F5' }"
                  />
                  <kendo-grid-column
                  ...
                  />
                  <kendo-grid-column
                    :field="$t('invdoReferenceNo')"
                    :title="$t('inv_do_number')"
                    :width="180"
                    :hidden="false"
                    :encoding="false"
                    :template="'<span>#=invdoReferenceNo || ``#</span>'"
                    :sortable="true"
                    :headerAttributes="{ style: 'background-color: #EDF1F5' }"
                  />
                  <kendo-grid-column
                  ...
                  <kendo-grid-column
                    ...
                    :width="200"
                    :sortable="false"
                    :template="'<span>#=kendo.toString(invAmount, decimalFormat)#</span>'"
                    :group-footer-template="'#= kendo.toString(sum, `n2`)#'"
                    :aggregates="['sum']"
                    :footerAttributes="{ style: 'text-align: right;' }"
                    :attributes="{ style: 'text-align: right; ' }"
        ...
        <div style="overflow: auto" class="bySO-pager"></div>
      </v-card>
    </v-col>
    <v-dialog persistent v-model="dialogSOStatus" max-width="450px">
      <v-card>
        <div class="modal_header">
          <v-card-title>{{ $t("change_status") }}: {{ soRefNum }}</v-card-title>
          <v-icon class="btn_close" @click="dialogSOStatus = false">close</v-icon>
        </div>
...
export default {
  ...
  data: () => ({
    ...
    aggregateDefinition: [
      ...
      { field: "invAmount", aggregate: "sum" },
      { field: "qAmount", aggregate: "sum" },
    ],
    params: {
      isDonor: false,
      search: "",
    ...
    changeStatus: ["Open", "Cancelled", "Invoiced"],
    status_: "",
    status__: "",
    status___: "",
    soRefNum: "",
  ...
  methods: {
          ...
          new Promise((resolve) => {
            setTimeout(() => {
              ...
              if (this.status___ === "Open") {
                status = 1;
              } else if (this.status___ === "Delivered") {
                status = 2;
              } else if (this.status___ === "Partially Delivered") {
                status = 3;
              } else if (this.status___ === "Cancelled") {
                status = 4;
              } else if (this.status___ === "Invoiced") {
                status = 5;
              } else if (this.status___ === "Partially Invoiced") {
                status = 6;
              }
    ...
    dataBound: function (e) {
      const grid = kendo.jQuery("#gridReportSaleOrderByCustomer").data("kendoGrid");
    ...
    pdfExport: function (e) {
      // const pivot = this.$refs.gridReportSaleOrderByCustomerDS.kendoWidget()
      // pivot.saveAsPDF()
    ...
    goToInvoice(e) {
      ...
      if (status === 6 || status === 1) {
        this.$router.push({
          path: "invoice",
          name: "Invoice",
          params: { id: this.soId, type: "saleorder" },
    ...
    goEdit(e) {
      ...
      if (dataItem.status === 1) {
        ...
        this.status__ = i18n.t("delivered");
      } else if (dataItem.status === 3) {
        this.status__ = i18n.t("partially_delivered");
        ...
        this.status__ = i18n.t("partially_invoiced");
      }
      if (dataItem.status === 1 || dataItem.status === 3 || dataItem.status === 6) {
        this.changeStatus = [];
        if (dataItem.status === 1) {
          ...
          this.status___ = i18n.t("delivered");
        } else if (dataItem.status === 6) {
          this.changeStatus = ["Invoiced"];
          this.status___ = i18n.t("invoiced");
        }
        this.dialogSOStatus = true;
      }
    },
    onSorterChanges(val) {
      let today = new Date();
    ...
    doReferenceTemplate(data) {
      const url = `delivery_order_view/${data.doTxnId}`;

      const refFrom = data.refFrom || [];
      const refTo = data.refTo || [];
      // let refs = []
      // if (data.doReferenceNo.includes('<br/>')) {
      //      const refNums = data.doReferenceNo.split('<br/>')
      //      for (const ref of refNums) {
      //       refs.push({
      //        url:`sale_order_view/${data.doTxnId}`,
      //        text: ref
      //       })
      //      }
      // } else {
      ...
      // }
      const args = {
        text: data.doReferenceNo,
        related: !!refFrom.length > 0 || !!refTo.length > 0,
        url: url,
        // refs: refs,
        data: data,
      };
      // window.console.log("data.qReferenceNo", data.qReferenceNo);
      return {
        template: LinkTemplate,
        templateArgs: args,
      };
    ...
    rowNumberTmpl(dataItem) {
      let ds = this.$refs.gridReportSaleOrderByCustomerDS.kendoWidget(),
    ...
    onSearch(type) {
      ...
      //     key: {},
      //   };
      this.searchTransaction("Sale Order funnel");
    ...
    status(dataItem) {
      ...
      switch (dataItem.status) {
        ...
        case 3:
          if (type === "Sale Order") {
            status = i18n.t("partally_delivered");
          } else {
        ...
        case 6:
          if (type === "Sale Order") {
            status = i18n.t("partally_invoiced");
          } else {
            status = i18n.t("void");
          }
          break;
    ...
    async loadPage(that) {
      $(".bySO-pager").html(`
                <label class="label bySO-item-returned">0</label>
                <div style="display: flex; flex-direction: 'row'">
                    <button class="bySO-next-page" title="previous" style="width: 24px; font-size: 19px;"> < </button>
                    <ul class="bySO-page-number" style="display: flex;list-style: none; padding-left: 0px;">
                        <li style="padding: 6px;"  data-page="1" class="bySO-page-link text-bold-link" >1</li>
                    </ul>
                    <button class="bySO-next-page" title="next" style="width: 24px; font-size: 19px;"> > </button>
                </div>
            `);
      $(document).on("click", ".bySO-page-link", function () {
        const page = $(this).data("page");
        $(".bySO-page-link").removeClass("text-bold-link");
        $(this).addClass("text-bold-link");
        that.backToPage(that, page - 1);
        window.console.log(page - 1);
      });
      // $(document).on('click', '.bySO-next-page', function () {
      $(".bySO-next-page").click(function () {
        const page_number = $(this)
          .parent()
          .children(".bySO-page-number")
          .find("li")
          .last()
          .data("page");
        window.console.log("bySO-page_number", page_number);
        ...
        if (isPrevious == "previous") {
          ...
          $(".bySO-page-link").removeClass("text-bold-link");
          $(`.bySO-page-link:eq(${currectpage - 1})`).addClass("text-bold-link");
          ...
          $(this)
            ...
            .children(".bySO-page-number")
            .append(
              `<li class="bySO-page-link text-bold-link" style="padding: 6px;" data-page="${
                page_number + 1
              }">${page_number + 1}</li>`
            );
        }
    ...
    itemRetuned(that) {
      ...
      $(".bySO-item-returned").text("Items returned (" + totalRow + ")");
    },
    disabledNextPage(value) {
      $(".bySO-next-page").prop("disabled", value);
    },
    ...
    refreshGridData(data) {
      $("#gridReportSaleOrderByCustomer").data("kendoGrid").dataSource.data([]);
      $("#gridReportSaleOrderByCustomer").data("kendoGrid").dataSource.data(data);
    },
