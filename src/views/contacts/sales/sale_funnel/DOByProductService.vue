      ...
      <v-card color="white" class="pa-3 no_border" elevation="0">
        <v-row>
          <v-col sm="8" cols="12" class="py-0">
            <h2 class="mb-0 font_020">{{ $t("do_by_product_service") }}</h2>
            <p class="mb-2">{{ $t("do_by_product_service_desc") }}</p>
          </v-col>
        </v-row>
        ...
        <v-row>
          <v-col sm="12" cols="12" class="py-0 pt-0">
            ...
            <gridFullScreen>
              <template>
                <kendo-datasource
                  ref="gridReportDOByProductServiceDS"
                  :type="'JSON'"
                  :data="transactions"
                  :group="group"
                  :server-paging="false"
                />
                <kendo-grid
                  id="gridReportDOByProductService"
                  class="grid-function"
                  :data-source-ref="'gridReportDOByProductServiceDS'"
                  :editable="false"
                  :groupable="true"
                  :sortable="true"
                  :column-menu="true"
                  ...
                  :excel-file-name="'ReportDOByProductService.xlsx'"
        ...
        <div style="overflow: auto" class="by-do-prodsrv-pager"></div>
      </v-card>
    </v-col>
    <!-- <v-dialog persistent v-model="dialogQuoteStatus" max-width="450px">
      <v-card>
        <div class="modal_header">
          ...
          <v-icon class="btn_close" @click="dialogQuoteStatus = false"
            >close</v-icon
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    dataBound: function (e) {
      const grid = kendo.jQuery("#gridReportDOByProductService").data("kendoGrid");
    ...
    pdfExport: function (e) {
      // const pivot = this.$refs.gridReportDOByProductServiceDS.kendoWidget()
      // pivot.saveAsPDF()
    ...
    doReferenceTemplate(data) {
      let url = `delivery_order_view/${data.id}`;
    ...
    quoteReferenceTemplate(data) {
      const url = `sale_quote_view/${data.soTxnId}`;
    ...
    rowNumberTmpl(dataItem) {
      let ds = this.$refs.gridReportDOByProductServiceDS.kendoWidget(),
    ...
    onSearch(type) {
      ...
      this.txnByEmployees = [];
      //   $(".k-pager-wrap").empty();
      //   $(".by-do-prodsrv-page-number").empty();
      this.loadPage(this);
      this.paging = {};
      //   this.params = {
      //     search: "",
    ...
    // },
    async loadPage(that) {
      $(".by-do-prodsrv-pager").html(`
                <label class="label by-do-prodsrv-item-returned">0</label>
                <div style="display: flex; flex-direction: 'row'">
                    <button class="by-do-prodsrv-next-page" title="previous" style="width: 24px; font-size: 19px;"> < </button>
                    <ul class="by-do-prodsrv-page-number" style="display: flex;list-style: none; padding-left: 0px;">
                        <li style="padding: 6px;"  data-page="1" class="by-do-prodsrv-page-link text-bold-link" >1</li>
                    </ul>
                    <button class="by-do-prodsrv-next-page" title="next" style="width: 24px; font-size: 19px;"> > </button>
                </div>
            `);
      $(document).on("click", ".by-do-prodsrv-page-link", function () {
        const page = $(this).data("page");
        $(".by-do-prodsrv-page-link").removeClass("text-bold-link");
        $(this).addClass("text-bold-link");
        that.backToPage(that, page - 1);
        window.console.log(page - 1);
      });
      // $(document).on('click', '.by-do-prodsrv-next-page', function () {
      $(".by-do-prodsrv-next-page").click(function () {
        const page_number = $(this)
          .parent()
          .children(".by-do-prodsrv-page-number")
          .find("li")
          .last()
          .data("page");
        window.console.log("by-do-prodsrv-page_number", page_number);
        ...
        if (isPrevious == "previous") {
          ...
          $(".by-do-prodsrv-page-link").removeClass("text-bold-link");
          $(`.by-do-prodsrv-page-link:eq(${currectpage - 1})`).addClass("text-bold-link");
          ...
          $(this)
            ...
            .children(".by-do-prodsrv-page-number")
            .append(
              `<li class="by-do-prodsrv-page-link text-bold-link" style="padding: 6px;" data-page="${
                page_number + 1
              }">${page_number + 1}</li>`
            );
        }
    ...
    itemRetuned(that) {
      ...
      $(".by-do-prodsrv-item-returned").text("Items returned (" + totalRow + ")");
    },
    disabledNextPage(value) {
      $(".by-do-prodsrv-next-page").prop("disabled", value);
    },
    ...
    refreshGridData(data) {
      $("#gridReportDOByProductService").data("kendoGrid").dataSource.data([]);
      $("#gridReportDOByProductService").data("kendoGrid").dataSource.data(data);
    },
