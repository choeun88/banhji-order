      ...
      <v-card color="white" class="pa-3 no_border" elevation="0">
        <v-row>
          <v-col sm="8" cols="12" class="py-0">
            <h2 class="mb-0 font_020">{{ $t("sale_quote_customer") }}</h2>
        ...
        <v-row>
          <v-col sm="12" cols="12" class="py-0 pt-0">
            ...
            <gridFullScreen>
              <template>
                <kendo-datasource
                  ref="gridReportQuoteByCustomerDS"
                  :type="'JSON'"
                  :data="transactions"
                  :group="group"
                  :server-paging="false"
                />
                <kendo-grid
                  id="gridReportQuoteByCustomer"
                  class="grid-function"
                  :data-source-ref="'gridReportQuoteByCustomerDS'"
                  :editable="false"
                  :groupable="true"
                  :sortable="true"
                  :column-menu="true"
                ...
                >
                  ...
                  <kendo-grid-column
                    ...
                    :command="[
                      { text: $t('convert'), click: goToSaleOrder },
        ...
        <div style="overflow: auto" class="byQuote-pager"></div>
      </v-card>
    </v-col>
...
export default {
  ...
  methods: {
          ...
          new Promise((resolve) => {
            setTimeout(() => {
              ...
              if (this.status___ === i18n.t("open")) {
                ...
                status = 2;
              } else if (this.status___ === i18n.t("abandoned")) {
                status = 3;
              } else if (this.status___ === i18n.t("declined")) {
                status = 4;
              }
    ...
    dataBound: function (e) {
      const grid = kendo.jQuery("#gridReportQuoteByCustomer").data("kendoGrid");
    ...
    pdfExport: function (e) {
      // const pivot = this.$refs.gridReportQuoteByCustomerDS.kendoWidget()
      // pivot.saveAsPDF()
    ...
    goToSaleOrder(e) {
      ...
      this.$router.push({
        path: "sale_order",
        name: "Sale Order",
        params: { id: this.quoteId, type: "quote" },
        query: { type: "quote" },
      });
    },
    goEdit(e) {
    ...
    rowNumberTmpl(dataItem) {
      let ds = this.$refs.gridReportQuoteByCustomerDS.kendoWidget(),
    ...
    async loadPage(that) {
      $(".byQuote-pager").html(`
                <label class="label byQuote-item-returned">0</label>
                <div style="display: flex; flex-direction: 'row'">
                    <button class="byQuote-next-page" title="previous" style="width: 24px; font-size: 19px;"> < </button>
                    <ul class="byQuote-page-number" style="display: flex;list-style: none; padding-left: 0px;">
                        <li style="padding: 6px;"  data-page="1" class="byQuote-page-link text-bold-link" >1</li>
                    </ul>
                    <button class="byQuote-next-page" title="next" style="width: 24px; font-size: 19px;"> > </button>
                </div>
            `);
      $(document).on("click", ".byQuote-page-link", function () {
        const page = $(this).data("page");
        $(".byQuote-page-link").removeClass("text-bold-link");
        $(this).addClass("text-bold-link");
        that.backToPage(that, page - 1);
        window.console.log(page - 1);
      });
      // $(document).on('click', '.byQuote-next-page', function () {
      $(".byQuote-next-page").click(function () {
        const page_number = $(this)
          .parent()
          .children(".byQuote-page-number")
          .find("li")
          .last()
          .data("page");
        window.console.log("byQuote-page_number", page_number);
        ...
        if (isPrevious == "previous") {
          ...
          $(".byQuote-page-link").removeClass("text-bold-link");
          $(`.byQuote-page-link:eq(${currectpage - 1})`).addClass("text-bold-link");
          ...
          $(this)
            ...
            .children(".byQuote-page-number")
            .append(
              `<li class="byQuote-page-link text-bold-link" style="padding: 6px;" data-page="${
                page_number + 1
              }">${page_number + 1}</li>`
            );
        }
    ...
    itemRetuned(that) {
      ...
      $(".byQuote-item-returned").text("Items returned (" + totalRow + ")");
    },
    disabledNextPage(value) {
      $(".byQuote-next-page").prop("disabled", value);
    },
    ...
    refreshGridData(data) {
      $("#gridReportQuoteByCustomer").data("kendoGrid").dataSource.data([]);
      $("#gridReportQuoteByCustomer").data("kendoGrid").dataSource.data(data);
    },
